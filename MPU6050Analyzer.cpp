...
void MPU6050Analyzer::setMagParam( float* fParam ,int iLen )
{
	for(int i=0;i<ACCPARAMCOUNT;i++)
		m_fMagParam[i] = fParam[i];
	m_bComDirtyMark = true;
}
...
void MPU6050Analyzer::getMagParam(float* fParam,int iLen)
{
	//assert(iLen == ACCPARAMCOUNT);
	for(int i=0;i<ACCPARAMCOUNT;i++)
		fParam[i] = m_fMagParam[i];
}
