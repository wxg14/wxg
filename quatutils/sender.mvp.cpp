...
Sender::Sender()
{
	...
	//for (int i=0; i<24; i++) // edit by mvp ## 2015-7-13
...
bool Sender::sendQuat(int portID, int segmentlabel, const osg::Quat& quat, bool cali, short moveflag, bool weightnessless)//准备驱动模型数据wxg6  moveflag为：加速度向量(扣除重力加速度后的)的模*1000
{
	...
	if (segmentlabel == 18)
	{
		...
		if (moveflag < 5)//the leg is stay...	// add by mvp ## 2015-7-20
			m_right_stay = true;
		else
			m_right_stay = false;
	}
	else if (segmentlabel == 22)
	{
		...
		if (moveflag < 10)//the leg is stay...
			m_left_stay = true;
		else
			m_left_stay = false;
	}	
...
//			m_hipPosIncy = 0;
		...
		//if(m_left_stay && m_right_stay )
		//{
		//	m_bonepos[18] = rightfootpos;
		//	m_bonepos[22] = leftfootpos;
		//}
		if(m_left_stay == true)
		{
			//m_bonepos[22].x() = m_bonepos[22].x() * 0.2 + leftfootpos.x() * 0.8;
			//m_bonepos[22].z() = m_bonepos[22].z() * 0.2 + leftfootpos.z() * 0.8;
			m_bonepos[22].x() = m_bonepos[22].x() * 0.1 + leftfootpos.x() * 0.9;
			m_bonepos[22].z() = m_bonepos[22].z() * 0.1 + leftfootpos.z() * 0.9;
		}
			//m_bonepos[18] = m_bonepos[18] * 0.5 + rightfootpos * 0.5;
		if(m_right_stay == true)
		{
			m_bonepos[18].x() = m_bonepos[18].x() * 0.2 + rightfootpos.x() * 0.8;
			m_bonepos[18].z() = m_bonepos[18].z() * 0.2 + rightfootpos.z() * 0.8;
		}
			//m_bonepos[22] = m_bonepos[22] * 0.5 + leftfootpos * 0.5;
	}
	m_segment[23].set(hpos.x() - hipspos.x(), hpos.y() - hipspos.y(), hpos.z() - hipspos.z(), 0);//根节点偏移 maya才用，已注去
	//m_segment[23].set(hpos.x(), hpos.y(), hpos.z(), 0);
	//hipspos = hpos;
...
/*
// add the two methord // add by mvp ## 2015-7-15
void Sender::calcPosition(bool cali)
{
	static bool rfootonfloor = false, lfootonfloor=false;
	static double velocity = 0; 
	...
	else//静止或行走
	{
		...
		case 1:	// the left stay and the right move
			...
			//m_bonepos[19].set(leftfootpos.x(),leftfootpos.y(),leftfootpos.z());
			m_bonepos[19].set(0.0,0.0,0.0);
			nseg = 19;
			while (nseg != 22)
			{
				m_bonepos[19] -= m_segment[m_parentbone[nseg]] * osg::Vec3f(m_bones[nseg].px, m_bones[nseg].py, m_bones[nseg].pz);//更新位移信息
				nseg = m_parentbone[nseg];
			}
			
			//PRINTF("%d\tX:\t%f\tY:\t%f\tZ:\t%f\n",0,(float)m_bonepos[0]._v[0],(float)m_bonepos[0]._v[1],(float)m_bonepos[0]._v[2]);
			//PRINTF("%d\tX:\t%f\tY:\t%f\tZ:\t%f\n",19,(float)m_bonepos[19]._v[0],(float)m_bonepos[19]._v[1],(float)m_bonepos[19]._v[2]);
			m_bonepos[0] = m_bonepos[19] + leftfootpos;
			hpos =  m_bonepos[0];
			PRINTF("AAAAAAAAAAA\n");
			hpos._v[1] += m_hipPosIncy;	// add the Incy of the Y distance
			m_hipPosIncy = 0;
		...
		case 2:	// the right stay and left move
			...
			//m_bonepos[15].set(rightfootpos.x(),rightfootpos.y(),rightfootpos.z());
			m_bonepos[15].set(0.0,0.0,0.0);
			nseg = 15;
			while (nseg != 18)
			{
				m_bonepos[15] -= m_segment[m_parentbone[nseg]] * osg::Vec3f(m_bones[nseg].px, m_bones[nseg].py, m_bones[nseg].pz);//更新位移信息
				nseg = m_parentbone[nseg];
			}			
			//PRINTF("%d\tX:\t%f\tY:\t%f\tZ:\t%f\n",0,(float)m_bonepos[0]._v[0],(float)m_bonepos[0]._v[1],(float)m_bonepos[0]._v[2]);
			//PRINTF("%d\tX:\t%f\tY:\t%f\tZ:\t%f\n",19,(float)m_bonepos[19]._v[0],(float)m_bonepos[19]._v[1],(float)m_bonepos[19]._v[2]);
			m_bonepos[0] = m_bonepos[15] + rightfootpos;
			hpos =  m_bonepos[0];
			PRINTF("BBBBBBBBBB\n");
			hpos._v[1] += m_hipPosIncy;	// add the Incy of the Y distance
			m_hipPosIncy = 0;
		...
		case 3: // stay for no move
			...
			//m_bonepos[0]._v[0] = (m_bonepos[15]._v[0] + m_bonepos[19]._v[0])/2.0;
			////m_bonepos[0]._v[1] = (leftfootpos.y() - lpos.y() + rightfootpos.y() - rpos.y())/2.0;// edit by mvp ## 2015-7-13
			//m_bonepos[0]._v[1] = (m_bonepos[15]._v[1] + m_bonepos[19]._v[1])/2.0;
			//m_bonepos[0]._v[2] = (m_bonepos[15]._v[2] + m_bonepos[19]._v[2])/2.0;
			m_bonepos[0] = (m_bonepos[15] + m_bonepos[19] + leftfootpos + rightfootpos)/2.0;
			m_bonepos[0]._v[1] += m_hipPosIncy;	// add the Incy of the Y distance
			m_hipPosIncy = 0;
			hpos = m_bonepos[0];
			m_parentbone[22] = 21; // calc the two leg
			m_parentbone[21] = 20;
			m_parentbone[20] = 19;
			m_parentbone[19] = 0;
