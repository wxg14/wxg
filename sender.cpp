...
#ifndef NDEBUG //debug
	*_tcsrchr(exepath,'\\') = '\0';
	strPath = QString("%1\\Release\\media\\models\\%2.xml").arg(exepath).arg(m_ModelName.toAscii().data());
#else//release
	strPath = QString("%1").arg(exepath) + QString("\\media\\models\\%1.xml").arg(m_ModelName.toAscii().data());
#endif
	QFile file(strPath);  
	if(!file.open(QIODevice::ReadWrite))  
	{
		QMessageBox box(QMessageBox::Warning,QString::fromLocal8Bit("路径无效"),QString::fromLocal8Bit(strPath.toAscii()));
